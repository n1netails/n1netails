  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    if (
      ...
      request.url.includes(
        `${this.authenticationService.host}/ninetails/auth/passkey/register/finish`
      ) ||
      request.url.includes(
        `${this.authenticationService.host}/ninetails/auth/passkey/login/start`
      ) ||
      request.url.includes(
        `${this.authenticationService.host}/ninetails/auth/passkey/login/finish`
      ) ||
    ...
    return next.handle(authorizationRequest).pipe(
      tap({
        next: () => {},
        error: (error: any) => {
